...
static void do_request(std::vector<std::string> &cmd, std::vector<uint8_t> &out) {
    if (cmd.size() == 2 && cmd[0] == "get") {
        ...
        if (it == g_data.end()) {
            make_response(std::vector<uint8_t>(), out, RES_NX);
            return;
        }
        ...
        make_response(data, out, RES_NX);
    } else if (cmd.size() == 3 && cmd[0] == "set") {
        g_data[cmd[1]].swap(cmd[2]);
    } else if (cmd.size() == 2 && cmd[0] == "del") {
        g_data.erase(cmd[1]);
    } else {
        make_response(std::vector<uint8_t>(), out, RES_ERR); // unrecognized command  
    }
}

static void make_response(std::vector<uint8_t> data, std::vector<uint8_t> &out, uint32_t status = 0) {
